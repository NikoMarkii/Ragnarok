 ...
 * - 複数のバリエーションが存在
 */
public class FairyEntity extends PathfinderMob implements GeoEntity, FlyingAnimal, Merchant {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(0, new Goal() {
            ...
            public boolean canUse() {
                return FairyEntity.this.getTradingPlayer() != null;
            }

            @Override
            public void start() {
                FairyEntity.this.getNavigation().stop();
            }

            @Override
            public void tick() {
                // 取引中は速度をゼロに固定して、空中静止させる
                FairyEntity.this.setDeltaMovement(Vec3.ZERO);
                // さらに、プレイヤーの方をしっかり向かせる
                FairyEntity.this.getLookControl().setLookAt(FairyEntity.this.getTradingPlayer(), 30.0F, 30.0F);
            }
        });
    }
    ...
    static class FairyPanicGoal extends Goal {
        ...
        public FairyPanicGoal(FairyEntity fairyEntity, double speedModifier) {
        ...
        public boolean canUse() {
            ...
            return this.fairyEntity.getLastHurtByMobTimestamp() > this.fairyEntity.tickCount - 100;
        }

        @Override
        public void start() {
            // ランダムな方向の上空へ逃げる
            Vec3 currentPos = this.fairyEntity.position();
            this.posX = currentPos.x + (this.fairyEntity.getRandom().nextDouble() - 0.5D) * 16.0D;
            this.posY = currentPos.y + 8.0D + this.fairyEntity.getRandom().nextDouble() * 4.0D;
            this.posZ = currentPos.z + (this.fairyEntity.getRandom().nextDouble() - 0.5D) * 16.0D;

            this.fairyEntity.setFleeing(true);
        }

        @Override
        public boolean canContinueToUse() {
            return !this.fairyEntity.getNavigation().isDone() &&
                    this.fairyEntity.getLastHurtByMobTimestamp() > this.fairyEntity.tickCount - 100;
        }

        @Override
        public void tick() {
            this.fairyEntity.getNavigation().moveTo(this.posX, this.posY, this.posZ, this.speedModifier);
        }
    ...
    static class FairyWanderGoal extends Goal {
        ...
        public FairyWanderGoal(FairyEntity fairyEntity) {
            this.fairyEntity = fairyEntity;
            this.setFlags(EnumSet.of(Flag.MOVE));
        }
        ...
        public boolean canUse() {
            ...
            return this.fairyEntity.getNavigation().isDone() &&
                    this.fairyEntity.getRandom().nextInt(100) == 0 &&
                    !this.fairyEntity.isFleeing();
        }

        @Override
        public boolean canContinueToUse() {
            return this.fairyEntity.getNavigation().isInProgress();
        }
        ...
        public void start() {
            ...
            if (target != null) {
                this.fairyEntity.getNavigation().moveTo(target.x, target.y, target.z, 1.0D);
            }
        }

        @Nullable
        private Vec3 getRandomPosition() {
            Vec3 current = this.fairyEntity.position();

            // 現在位置から8ブロック以内のランダムな位置
            double x = current.x + (this.fairyEntity.getRandom().nextDouble() - 0.5D) * 16.0D;
            double y = current.y + (this.fairyEntity.getRandom().nextDouble() - 0.5D) * 4.0D;
            double z = current.z + (this.fairyEntity.getRandom().nextDouble() - 0.5D) * 16.0D;

            // 地面から最低3ブロック以上の高さを保つ
            BlockPos groundPos = new BlockPos((int)x, (int)y, (int)z);
            while (groundPos.getY() > this.fairyEntity.level().getMinBuildHeight() &&
                    !this.fairyEntity.level().getBlockState(groundPos).isSolidRender(this.fairyEntity.level(), groundPos)) {
